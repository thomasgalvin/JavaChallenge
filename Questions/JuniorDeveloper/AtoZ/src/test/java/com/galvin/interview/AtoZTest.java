...
public class AtoZTest
{
    ...
    public void testAtoZ() throws Exception {
        ...
        for( String test : CORRECT_UPPERCASE ) {
            Assert.assertTrue( "Failed to match", atoz.match( test ) );
            Assert.assertTrue( "Failed to match", atoz.matchIgnoreCase( test ) );
        }
        
        for( String test : CORRECT_LOWERCASE ) {
            Assert.assertFalse( "Incorrectly matched", atoz.match( test ) );
            Assert.assertTrue( "Failed to match", atoz.matchIgnoreCase( test ) );
        }
